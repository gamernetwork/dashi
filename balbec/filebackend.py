...
e.args[1]))
    ...
    def parseLastcheck(self, text):
        ...
        pattern = re.compile('programstatus \{[^\}]*?last_command_check=(\S+)\n[^\}]*?\}', re.DOTALL)
    ...
    def parseHostgroups(self, text):
        ...
        pattern = re.compile('define hostgroup \{[^\}]*?hostgroup_name\s+([\S ]+)\n[^\}]*?members\s+([\S ]+)\n[^\}]*?\}', re.DOTALL)
    ...
    def parseServicegroups(self, text):
        ...
        pattern = re.compile('define servicegroup \{[^\}]*?servicegroup_name\s+([\S ]+)\n[^\}]*?members\s+([\S ]+)\n[^\}]*?\}', re.DOTALL)
    ...
    def parseHosts(self, text):
        ...
        #pattern = re.compile('hoststatus'+' \{[^\}]*?host_name=(\S+)\n[^\}]*?current_state=(\S+)\n[^\}]*?plugin_output=([\S ]+)\n[^\}]*?\}', re.DOTALL) 
        pattern = re.compile('hoststatus'+' \{[^\}]*?host_name=(\S+)\n[^\}]*?current_state=(\S+)\n[^\}]*?plugin_output=([^\n]+)\n[^\}]*?\}', re.DOTALL) 
    ...
    def parseServicestati(self, text):
        ...
        pattern = re.compile('servicestatus'+' \{[^\}]*?host_name=(\S+)\n[^\}]*?service_description=([\S ]+)\n[^\}]*?current_state=(\S+)\n[^\}]*?plugin_output=([\S ]+)\n[^\}]*?\}', re.DOTALL)
